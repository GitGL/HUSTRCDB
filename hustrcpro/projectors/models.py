...
class Projector(models.Model):
    ...
    def __str__(self):
        ...
        return str(self.projector_id) + str(': ') + self.brand.brand_cn + str(': ') + self.brand.model_number + str(': ') + self.serial_no
...
class RoomManager(models.Manager):
    def room_projectors(self, build_id):
        cursor = connection.cursor()
        cursor.execute("""select r.build_id, r.room_id, r.room_name, p.projector_id, b.brand_id, b.brand_cn, b.brand_en, b.model_number, vps.status_id, vps.status_name, vps.date_register from Rooms r left join Projectors p on r.room_id = p.room_id left join Brands b on p.brand_id = b.brand_id left join view_projector_status vps on p.projector_id = vps.projector_id where r.build_id = %s""", [build_id])
